    ...
    override fun run(action: SingleAction, target: ActionController.TargetInfo?, button: View?): Boolean {
        ...
        when (action.id) {
            ...
            SingleAction.SHARE_WEB -> {
                val tab = controller.getTab(actionTarget)
                    //TODO create deep link here
                val builder = Uri.Builder()
                    .scheme("https")
                    .authority("appyhigh.page.link")
                    .path("/")
                    .appendQueryParameter("url", tab.url)

                val shortLinkTask = Firebase.dynamicLinks.shortLinkAsync {
                    link = builder.build()
                    domainUriPrefix = "https://appyhigh.page.link"
                    // Set parameters
                    androidParameters("jp.hazuki.yuzubrowser.debug") {
                        minimumVersion = 1
                    }
                    // ...
                    ...
                    // Short link created
                    WebUtils.shareWeb(controller.activity, it.shortLink.toString(), tab.title)
